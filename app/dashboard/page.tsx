    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-100 font-sans p-4 md:p-8 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto space-y-8">
        ...
        <div className="flex gap-2 p-1 bg-slate-900/50 rounded-xl w-full md:w-fit border border-slate-700 backdrop-blur-md overflow-x-auto">
            <TabButton label="Cone Production" icon={<Database size={16}/>} active={activeTab === 'production'} onClick={() => setActiveTab('production')} />
            <TabButton label="Quality Check" icon={<CheckCircle2 size={16}/>} active={activeTab === 'quality'} onClick={() => setActiveTab('quality')} />
            <TabButton label="Equal Team" icon={<Users size={16}/>} active={activeTab === 'equal'} onClick={() => setActiveTab('equal')} />
        </div>
            ...
            {activeTab === 'equal' && (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
                    ...
                    <div className="grid md:grid-cols-3 gap-6">
                        <BigCard label="Total Boxes Checked" value={metrics?.BoxesChecked} icon={<Box size={32}/>} color="blue" />
                        <BigCard label="Rejected Pieces" value={metrics?.EqualRejected} icon={<AlertTriangle size={32}/>} color="red" />
                        <BigCard label="Ready for Packing" value={metrics?.EqualPacking} icon={<Package size={32}/>} color="green" />
                    </div>
                </div>
            )}
        </div>
...
function TabButton({ label, icon, active, onClick }: any) {
    return (
        <button onClick={onClick} className={`flex items-center gap-2 px-6 py-3 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${active ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50 scale-[1.02]' : 'text-slate-400 hover:text-white hover:bg-slate-800'}`}>
            {icon} {label}
        </button>
    );
}
...
function TableCard({ title, children }: any) {
    return (
        <div className="bg-slate-900/30 rounded-2xl border border-slate-700/50 overflow-hidden">
            ...
            <div className="p-2"><table className="w-full text-sm"><tbody>{children}</tbody></table></div>
        </div>
    );
}
...
function MetricRow({ label, value, unit, highlight = false, isBad = false }: any) {
    ...
    return (
      <tr className="border-b border-slate-700/30 last:border-0 hover:bg-white/5 transition-colors group">
        ...
        <td className={`p-4 text-right font-bold ${highlight ? 'text-xl text-green-400' : isBad ? 'text-red-400' : 'text-white'} ${isLongText ? 'text-xs leading-tight max-w-[150px]' : ''}`}>
    ...
    return (
        <div className="bg-slate-900/40 p-6 rounded-2xl border border-slate-700 shadow-inner">
            <div className="flex justify-between mb-3 items-end">
                ...
                <div className="text-right"><span className="text-2xl font-black text-white">{safeCurrent}</span><span className="text-sm text-slate-500 mx-1">/</span><span className="text-sm font-bold text-slate-400">{safeTotal}</span></div>
            </div>
            <div className="h-4 bg-slate-800 rounded-full overflow-hidden">
                <div className={`h-full bg-blue-500 transition-all duration-1000 ease-out shadow-[0_0_15px_rgba(59,130,246,0.6)]`} style={{ width: `${percent}%` }}><div className="w-full h-full bg-white/20 animate-pulse"></div></div>
            </div>
